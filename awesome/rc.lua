...
battery_widget.font = "Arial 10" -- Adjust the font and font size as desired
battery_widget.align = "right"
battery_widget.valign = "center"

-- Function to update the battery percentage
local function update_battery(widget)
    local handle = io.popen("upower -i /org/freedesktop/UPower/devices/battery_BAT0 | awk '/percentage:/ {print substr($2, 1, length($2)-1)}'")
    local battery_percentage = handle:read("*a"):gsub("\n", "")
    handle:close()
    widget.text = "Battery: " .. battery_percentage .. "%"
end
...
-- Update the battery percentage every 60 seconds
gears.timer {
    timeout = 60,
    autostart = true,
    call_now = true,
    callback = function() update_battery(battery_widget) end
}

-- Add the battery widget to the right side of the top bar
-- table.insert(s.mywibox.widgets, battery_widget)
